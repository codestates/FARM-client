...
export default function SignUp({ handleLoginWindow, handleLoginSuccess }) {
  ...
  const handleInputValue = (key) => (e) => {
    if (key === "email") {
      ...
      if (valid) setEmailMessage("유효한 이메일 입니다.");
      else setEmailMessage("유효하지 않은 이메일 입니다.");
      setEmail(e.target.value);
    }
    if (key === "name") {
      let valid = isValidName(e.target.value);
      if (valid) setNameMessage("유효한 이름 입니다.");
      else setNameMessage("2~4자 한글만 허용합니다.");
      setName(e.target.value);
    }
    if (key === "pw") {
      let valid = isValidPw(e.target.value);
      if (valid) setPwMessage("유효한 비밀번호 입니다.");
      else setPwMessage("8~10자 영문,숫자 조합만 허용합니다.");
      setPw(e.target.value);
    }
    if (key === "pwCheck") {
      if (pw === e.target.value) setPwCheckMessage("비밀번호가 일치합니다.");
  ...
  const isValidEmail = (email) => {
    const regExp = /^[0-9a-zA-Z]{3,}([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i;
    return regExp.test(email); // 영어or숫자 3-20자 + @ + 영어 + . + 영어2-3자
  };
  const isValidName = (name) => {
    const regExp = /^[가-힣]{2,4}$/; // 2~4자 한글만
    return regExp.test(name);
  };
  const isValidPw = (pw) => {
    const regExp = /^(?=.*\d)(?=.*[a-zA-Z])[0-9a-zA-Z]{8,10}$/; // 8~10자 영문-숫자 조합만 허용
  ...
  return (
    <div className="Login_Window">
      ...
      <form onSubmit={handleSignUp} className="Login_Wrapper">
